    ...
    public static function resume_kpi($mtd, $where_loc, $scope)
    {
        ...
        $resume = DB::select(
            "SELECT a.$scope,
                ...
                COALESCE(SUM(d.trade),0) AS trade,
                COALESCE(SUM(d.last_trade),0) AS last_trade,
                ...
                COALESCE(SUM(g.oss_osk),0) AS oss_osk,
                COALESCE(SUM(g.last_oss_osk),0) AS last_oss_osk,
                ...
                LEFT JOIN (SELECT telp,COUNT(CASE WHEN date BETWEEN '$m1' AND '$mtd' THEN msisdn END ) trade, COUNT(CASE WHEN date BETWEEN '$last_m1' AND '$last_mtd' THEN msisdn END ) last_trade FROM sales_copy WHERE date BETWEEN '$last_m1' AND '$mtd' AND kategori='TRADE IN' GROUP BY 1) d ON a.telp=d.telp
