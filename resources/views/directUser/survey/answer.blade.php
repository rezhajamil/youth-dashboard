...
<section class="flex justify-center w-full h-full min-h-screen px-4 py-4 bg-premier">
    <div class="w-full px-4 py-2 bg-white rounded-lg shadow-xl h-fit sm:w-3/4 ">
        ...
        <span class="block w-full font-bold text-center text-tersier">Hasil Survey Anda : {{ $answer->hasil }}/{{ count(json_decode($survey->soal)) }}</span>
        <div class="p-4 px-8 mx-auto my-8 rounded-full bg-sekunder w-fit">
            <span class="block w-full py-2 text-2xl font-bold text-center text-white">{{ number_format(($answer->hasil/count(json_decode($survey->soal)))*100,0,".",",") }}</span>
        </div>
        @else
        ...
        <input type="hidden" id="time-end" value="{{ date('M d, Y H:i:s', strtotime("+".$survey->time." minutes", strtotime($answer->time_start))) }}">
        @endif
        @else
        <span class="block w-full font-bold text-sekunder">Selamat Datang {{ $user->nama }} | {{ $user->telp }}</span>
        @if($survey)
        <span class="block w-full my-2 font-semibold text-tersier">Waktu Mengerjakan Survey : {{ $survey->time }} Menit</span>
        {!! $survey->deskripsi !!}
        <a href="{{ URL::to('/start/survey?telp='.request()->get('telp')) }}" class="block px-4 py-2 mx-auto my-6 font-semibold text-white transition-all rounded bg-sekunder w-fit hover:bg-black">
            Mulai Survey
        </a>
        @endif
        @endif
