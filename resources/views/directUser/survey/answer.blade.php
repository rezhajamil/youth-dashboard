...
@section('body')
    <section class="flex justify-center w-full h-full min-h-screen px-4 py-4 bg-premier">
        <div class="w-full px-4 py-2 bg-white rounded-lg shadow-xl h-fit sm:w-3/4 ">
            @if ($survey)
                <span
                    class="block w-full py-2 mb-2 text-2xl font-bold text-center border-b-2 text-sekunder">{{ $survey ? $survey->nama : '' }}</span>
                @if ($answer && $answer->finish)
                    <span class="block w-full mt-4 mb-2 text-xl font-bold text-center text-tersier">Survey Telah
                        Selesai</span>
