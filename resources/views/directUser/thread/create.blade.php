...
@section('body')
    <section class="flex flex-col w-full h-full min-h-screen px-2 py-4 bg-gray-100 font-caregold">
        ...
        <a href="{{ route('thread.index', ['telp' => request()->get('telp')]) }}"
            class="px-3 py-1 text-sm font-semibold text-white transition-all rounded-md w-fit bg-gradient-to-br from-y_tersier to-y_tersier/60 hover:to-y_tersier">
            <i class="mr-2 text-white fa-solid fa-arrow-left"></i>
        ...
        <div class="px-3 py-2 my-6 bg-white rounded-md shadow-xl">
            <form action="{{ route('thread.store') }}" class="" method="POST">
                ...
                <trix-editor input="message" class="h-72"></trix-editor>
                @error('message')
