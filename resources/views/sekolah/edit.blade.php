                ...
                <form action="{{ route('sekolah.update',$sekolah->NPSN) }}" method="POST" class="">
                    ...
                    <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                        <div class="w-full">
                            ...
                            <input class="w-full rounded-md form-input focus:border-indigo-600" type="text" name="latitude" value="{{ old('latitude',$sekolah->latitude) }}">
                            @error('latitude')
                        ...
                        <div class="w-full">
                            ...
                            <input class="w-full rounded-md form-input focus:border-indigo-600" type="text" name="longitude" value="{{ old('longitude',$sekolah->longitude) }}">
                            @error('longitude')
                            <span class="block mt-1 text-sm italic text-red-600">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="w-full">
                            ...
                            <select name="pjp" id="pjp" class="w-full rounded-md" required>
                                ...
                                <option value="NON PJP"  {{old('pjp',$sekolah->PJP)=='NON PJP'?'selected':''}}>NON PJP</option>
                            </select>
                            @error('pjp')
                            <span class="block mt-1 text-sm italic text-red-600">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="w-full">
                            ...
                            <select name="frekuensi" id="frekuensi" class="w-full rounded-md" required>
                                ...
                                <option value="F1" {{old('frekuensi',$sekolah->FREKUENSI)=='F1'?'selected':''}}>F1</option>
                                <option value="F2" {{old('frekuensi',$sekolah->FREKUENSI)=='F2'?'selected':''}}>F2</option>
                                <option value="F3" {{old('frekuensi',$sekolah->FREKUENSI)=='F3'?'selected':''}}>F3</option>
                                <option value="F4" {{old('frekuensi',$sekolah->FREKUENSI)=='F4'?'selected':''}}>F4</option>
                            </select>
                            @error('frekuensi')
                            <span class="block mt-1 text-sm italic text-red-600">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="w-full">
                            ...
                            <select name="telp" id="telp" class="w-full rounded-md" required>
                                ...
                                <option value="{{$data->telp}}" {{old('telp',$sekolah->TELP)==$data->telp?'selected':''}}>{{$data->nama}} | {{$data->telp}}</option>
                                @endforeach
                            </select>
                            @error('telp')
                            <span class="block mt-1 text-sm italic text-red-600">{{ $message }}</span>
