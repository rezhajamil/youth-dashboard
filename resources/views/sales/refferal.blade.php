@extends('layouts.dashboard.app', ['plain' => true, 'title' => 'Report Trade In Buddies'])
@section('body')
    <section class="flex h-full min-h-screen w-full flex-col items-center bg-indigo-400 px-4 py-4">
        <span class="mb-6 mt-2 inline-block w-full text-left font-bold text-white">Report Trade In Buddies
            <br>
        ...
        <div class="my-4 h-fit w-full rounded-lg bg-white px-4 py-2 shadow-xl sm:w-3/4">
            @if (request()->get('telp'))
                ...
                @else
                    <span class="inline-block w-full text-center font-bold text-slate-500">Report Trade In Buddies</span>
                    <form action="{{ route('sales.store_refferal') }}" method="post">
                        ...
                        <button type="submit" id="btn-submit"
                            class="my-2 w-full rounded bg-indigo-800 px-6 py-2 font-semibold text-white">Submit</button>
                        <a href="{{ route('sales.get_refferal') }}"
            ...
            @else
                <form action="{{ route('sales.get_refferal') }}" method="get">
                    ...
                    <input class="form-input mt-2 w-full rounded-md focus:border-indigo-600" type="number" name="telp"
                        id="telp" placeholder="Masukkan Telepon / USER ID Buddies (6281234xxxxx)"
                        value="{{ old('telp') }}" required>
                    <button type="submit" id="btn-submit"
                        class="my-4 w-full rounded bg-indigo-800 px-6 py-2 font-semibold text-white">Submit</button>
                </form>
            @endif
        </div>
    </section>
