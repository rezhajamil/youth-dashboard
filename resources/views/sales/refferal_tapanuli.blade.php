@extends('layouts.dashboard.app', ['plain' => true, 'title' => 'Report Trade In Tapanuli'])
@section('body')
    <section class="flex h-full min-h-screen w-full flex-col items-center bg-teal-400 px-4 py-4">
        <span class="mb-6 mt-2 inline-block w-full text-left font-bold text-white">Report Trade In Tapanuli
            <br>
        ...
        @if (request()->get('telp') && $user)
            <div class="my-4 h-fit w-full rounded-lg bg-teal-600 px-4 py-2 shadow-xl sm:w-3/4">
        ...
        <div class="my-4 h-fit w-full rounded-lg bg-white px-4 py-2 shadow-xl sm:w-3/4">
            @if (request()->get('telp'))
                ...
                @else
                    <span class="inline-block w-full text-center font-bold text-slate-500">Report Trade In Tapanuli</span>
                    <form action="{{ route('sales.store_refferal_tapanuli') }}" method="post">
                        @csrf
                        ...
                        <button type="submit" id="btn-submit"
                            class="my-2 w-full rounded bg-teal-800 px-6 py-2 font-semibold text-white">Submit</button>
                        <a href="{{ route('sales.get_refferal_tapanuli') }}"
                ...
                @endif
            @else
                <form action="{{ route('sales.get_refferal_tapanuli') }}" method="get">
                    <span class="font-semibold text-sekunder">Telepon Agent</span>
                    <input class="form-input mt-2 w-full rounded-md focus:border-teal-600" type="number" name="telp"
                        id="telp" placeholder="Masukkan Telepon / USER ID Agent (812345xxxx)"
                        value="{{ old('telp') }}" required>
                    <button type="submit" id="btn-submit"
                        class="my-4 w-full rounded bg-teal-800 px-6 py-2 font-semibold text-white">Submit</button>
                </form>
            @endif
        </div>
    </section>
