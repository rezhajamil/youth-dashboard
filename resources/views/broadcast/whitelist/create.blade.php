                    ...
                    <form action="{{ route('whitelist.store') }}" method="POST" class="" enctype="multipart/form-data">
                                ...
                                <select name="cluster" id="cluster" class="w-full rounded-md" required>
                                    ...
                                    @foreach ($cluster as $item)
                                        <option value="{{ $item->cluster }}"
                                            {{ old('cluster') == $item->cluster ? 'selected' : '' }}>{{ $item->cluster }}</option>
                                    @endforeach
                                </select>
                                @error('cluster')
                        ...
                        @enderror
                                ...
                                <select name="program" id="program" class="w-full rounded-md" required>
                                    ...
                                    @foreach ($dataProgram as $item)
                                        <option value="{{ $item->program }}"
                                            {{ old('program') == $item->program ? 'selected' : '' }} class="program">
                                            {{ $item->program }}</option>
                                    @endforeach
                                </select>
                                @error('program')
                            ...
                            <div>
                                ...
                                <label class="block text-xs text-gray-700" for="file">Format 'msisdn;site_id ->
                                    628xxxxxxxxxxx;xxxxx'</label>
                                <label class="block text-xs text-gray-700" for="file">File maksimal 1000 baris</label>
                            ...
                            <div class="w-full p-2">
                                ...
                                <img src="{{ asset('images/contoh.jpg') }}" alt="contoh" class="object-contain border-2">
                            </div>
                        </div>
