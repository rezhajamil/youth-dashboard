            ...
            <div class="mt-4">
                @if (Auth::user()->privilege != 'cluster')
                    <a href="{{ route('whitelist.create') }}"
                        class="inline-block px-4 py-2 my-2 font-bold text-white bg-y_premier transition-all rounded-md hover:bg-y_premier"><i
                            class="mr-2 fa-solid fa-plus"></i> Data Whitelist Baru</a>
                    <a href="{{ route('whitelist.distribusi.create') }}"
                        class="inline-block px-4 py-2 my-2 font-bold text-white bg-orange-600 transition-all rounded-md hover:bg-orange-800"><i
                            class="mr-2 fa-solid fa-plus"></i> Distribusi Whitelist</a>
                ...
                <div class="flex justify-between mt-4 ">
                    <form class="flex flex-wrap items-center gap-x-4 gap-y-2" action="{{ route('whitelist.index') }}"
                        method="get">
                        ...
                        <select name="program_call" id="program_call">
                            ...
                            @foreach ($dataProgramCall as $item)
                                <option value="{{ $item->program }}"
                                    {{ request()->get('program_call') == $item->program ? 'selected' : '' }}>
                                    {{ $item->program }}
                                </option>
                            @endforeach
                        </select>
                        <div class="flex gap-x-3">
                            <button
                                class="px-4 py-2 font-bold text-white transition bg-y_premier rounded-lg hover:bg-y_premier"><i
                ...
                <div class="overflow-hidden bg-white rounded-md shadow w-fit table-broadcast">
                    <table class="text-left border-collapse w-fit">
                        <thead class="border-b">
                            <tr>
                                <th rowspan="2"
                                    class="p-3 text-sm font-bold text-gray-100 uppercase border-2 border-tersier bg-premier">
                                    No</th>
                                <th rowspan="2"
                                    class="p-3 text-sm font-medium text-center text-gray-100 uppercase border-2 border-tersier bg-premier">
                                    Branch</th>
                                <th colspan="3"
                                    class="p-3 text-sm font-medium text-center text-gray-100 uppercase border-2 border-tersier bg-premier">
                                    Whitelist</th>
                            ...
                            <tr>
                                <th
                                    class="p-3 text-sm font-medium text-center text-gray-100 uppercase border-2 border-tersier bg-sekunder">
                                    Total</th>
                                <th
                                    class="p-3 text-sm font-medium text-center text-gray-100 uppercase border-2 border-tersier bg-sekunder">
                                    Diambil</th>
                        ...
                        <tbody>
                            @foreach ($whitelist_branch as $key => $data)
                                <tr class="transition hover:bg-gray-200/40">
                                    ...
                                    <td class="p-3 text-gray-700 uppercase border-2 whitespace-nowrap">
                                        {{ number_format(($data->diambil / $data->wl) * 100, 2) . ' %' }}</td>
                                    <td class="p-3 text-gray-700 uppercase border-2 ">{{ $data->sudah }}</td>
                                    <td class="p-3 text-gray-700 uppercase border-2 ">{{ $data->belum }}</td>
                                    <td class="p-3 text-gray-700 uppercase border-2 whitespace-nowrap">
                                        {{ number_format(($data->sudah / $data->wl) * 100, 2) . ' %' }}</td>
                        ...
                        <thead class="border-b">
                            <tr>
                                ...
                                <th colspan="4"
                                    class="p-3 text-sm font-medium text-center text-gray-100 uppercase border-2 border-tersier bg-premier">
                                    Broadcast</th>
                                @if (Auth::user()->privilege == 'branch' || Auth::user()->privilege == 'superadmin')
                                    <th rowspan="2"
                                        class="p-3 text-sm font-medium text-center text-gray-100 uppercase border-2 border-tersier bg-premier">
                                        Action</th>
                                @endif
                                    ...
                                    @if (Auth::user()->privilege == 'branch' || Auth::user()->privilege == 'superadmin')
                                        <td class="p-3 text-gray-700 border-b">
                                            <form action="{{ route('whitelist.release', $data->telp) }}" method="post">
                                                ...
                                                <button
                                                    class="block my-1 text-base font-semibold text-left text-green-600 transition hover:text-green-800">Release
                                                    Whitelist</button>
                                            </form>
                                        </td>
                                    @endif
